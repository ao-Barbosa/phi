...
cleanup() {
	...
	case "$test_root" in
		"$temp_parent"/phi-test.*)
			if [[ -d "$test_root" && ! -L "$test_root" && -f "$test_root/.phi-test-owned" ]]; then
				rm -rf -- "$test_root"
			else
...
test_env=(
	...
	"BUN_INSTALL_CACHE_DIR=$test_root/cache/bun"
	"PHI_NO_LOCAL_LLM=1"
