...
#   ln -s "$PWD/scripts/auto-phi.sh" "$HOME/.local/bin/phi"
#
# ~/.local/bin must appear before the stable phi installation on PATH.

# Resolve this script through symlinks so repo_dir points at the development
# checkout rather than the directory containing the `phi` symlink.
...
if [[ "$use_stable" == true ]]; then
	if ! stable_phi="$(find_stable_phi)"; then
		echo "error: could not find a stable phi executable after the auto-phi wrapper on PATH" >&2
		exit 1
	fi
	exec "$stable_phi" ${args[@]+"${args[@]}"}
fi
...
export PHI_EXPERIMENTAL="${PHI_EXPERIMENTAL:-1}"
exec "$dev_phi" ${args[@]+"${args[@]}"}
