...
const MINIMUM_PHI_VERSION = "0.80.7";
...
function buildIndex(existingIndex, publication) {
	const entry = {
		minimumPiVersion: MINIMUM_PHI_VERSION,
	...
	const catalogs = (existingIndex?.catalogs || [])
		.filter((catalog) => catalog.minimumPiVersion !== MINIMUM_PHI_VERSION)
		.concat(entry)
...
async function main() {
	...
	try {
		...
		const currentEntry = currentIndex?.catalogs.find(
			(catalog) => catalog.minimumPiVersion === MINIMUM_PHI_VERSION,
		);
