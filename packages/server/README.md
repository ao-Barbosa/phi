...
import { MemorySessionRepo, type Session } from "@ao-Barbosa/phi-agent-core";
import {
...
} from "@ao-Barbosa/phi-server";
import { createUnixServer, getUnixSocketPath } from "@ao-Barbosa/phi-server/unix";
...
async function startServer(
  ...
  const server = createUnixServer(host, {
    serverId,
    path: getUnixSocketPath(serverId, "/run/user/1000/phi"),
  });
  await server.start();
  return server;
}
