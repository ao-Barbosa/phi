...
describe("totalTokens field", () => {
	...
	describe.skipIf(!process.env.ANTHROPIC_APHI_KEY)("Anthropic (API Key)", () => {
		it(
			...
			async () => {
				...
				console.log(`\nAnthropic / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.ANTHROPIC_APHI_KEY });
	...
	describe.skipIf(!process.env.XAI_APHI_KEY)("xAI", () => {
		it("grok-4.3 - should return totalTokens equal to sum of components", { retry: 3, timeout: 60000 }, async () => {
			...
			console.log(`\nxAI / ${llm.id}:`);
			const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.XAI_APHI_KEY });
	...
	describe.skipIf(!process.env.GROQ_APHI_KEY)("Groq", () => {
		it(
			...
			async () => {
				...
				console.log(`\nGroq / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.GROQ_APHI_KEY });
	...
	describe.skipIf(!process.env.CEREBRAS_APHI_KEY)("Cerebras", () => {
		it(
			...
			async () => {
				...
				console.log(`\nCerebras / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.CEREBRAS_APHI_KEY });
	...
	describe.skipIf(!hasCloudflareWorkersAICredentials())("Cloudflare Workers AI", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.CLOUDFLARE_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.TOGETHER_APHI_KEY)("Together AI", () => {
		it("Kimi-K2.6 - should return totalTokens equal to sum of components", { retry: 3, timeout: 60000 }, async () => {
			...
			console.log(`\nTogether AI / ${llm.id}:`);
			const { first, second } = await testTotalTokensWithCache(llm, {
				apiKey: process.env.TOGETHER_APHI_KEY,
	...
	describe.skipIf(!process.env.BASETEN_APHI_KEY)("Baseten", () => {
		it("GLM 5.2 - should return totalTokens equal to sum of components", { retry: 3, timeout: 60000 }, async () => {
			...
			console.log(`\nBaseten / ${llm.id}:`);
			const { first, second } = await testTotalTokensWithCache(llm, {
				apiKey: process.env.BASETEN_APHI_KEY,
				reasoningEffort: "high",
			});
	...
	describe.skipIf(!process.env.ZAI_APHI_KEY)("z.ai", () => {
		it("glm-5.2 - should return totalTokens equal to sum of components", { retry: 3, timeout: 60000 }, async () => {
			...
			console.log(`\nz.ai / ${llm.id}:`);
			const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.ZAI_APHI_KEY });
	...
	describe.skipIf(!process.env.MISTRAL_APHI_KEY)("Mistral", () => {
		it(
			...
			async () => {
				...
				console.log(`\nMistral / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.MISTRAL_APHI_KEY });
	...
	describe.skipIf(!process.env.MINIMAX_APHI_KEY)("MiniMax", () => {
		it(
			...
			async () => {
				...
				console.log(`\nMiniMax / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.MINIMAX_APHI_KEY });
	...
	describe.skipIf(!process.env.XIAOMI_APHI_KEY)("Xiaomi MiMo (API billing)", () => {
		it(
			...
			async () => {
				...
				console.log(`\nXiaomi MiMo / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.XIAOMI_APHI_KEY });
	...
	describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_CN_APHI_KEY)("Xiaomi MiMo Token Plan (CN)", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.XIAOMI_TOKEN_PLAN_CN_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_AMS_APHI_KEY)("Xiaomi MiMo Token Plan (AMS)", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.XIAOMI_TOKEN_PLAN_AMS_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.XIAOMI_TOKEN_PLAN_SGP_APHI_KEY)("Xiaomi MiMo Token Plan (SGP)", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.XIAOMI_TOKEN_PLAN_SGP_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.QWEN_TOKEN_PLAN_APHI_KEY)("Qwen Token Plan", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.QWEN_TOKEN_PLAN_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.QWEN_TOKEN_PLAN_CN_APHI_KEY)("Qwen Token Plan (CN)", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, {
					apiKey: process.env.QWEN_TOKEN_PLAN_CN_APHI_KEY,
				});
	...
	describe.skipIf(!process.env.KIMI_APHI_KEY)("Kimi For Coding", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.KIMI_APHI_KEY });
	...
	describe.skipIf(!process.env.AI_GATEWAY_APHI_KEY)("Vercel AI Gateway", () => {
		it(
			...
			async () => {
				...
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.AI_GATEWAY_APHI_KEY });
	...
	describe.skipIf(!process.env.OPENROUTER_APHI_KEY)("OpenRouter", () => {
		it(
			...
			async () => {
				...
				console.log(`\nOpenRouter / ${llm.id}:`);
				const { first, second } = await testTotalTokensWithCache(llm, { apiKey: process.env.OPENROUTER_APHI_KEY });
