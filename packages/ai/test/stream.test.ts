...
describe("Generate E2E Tests", () => {
	describe.skipIf(!process.env.GEMINI_APHI_KEY)("Gemini Provider (gemini-2.5-flash)", () => {
	...
	describe("Google Vertex Provider (gemini-3-flash-preview)", () => {
		...
		const vertexApiKey = process.env.GOOGLE_CLOUD_APHI_KEY;
	...
	describe.skipIf(!process.env.DEEPSEEK_APHI_KEY)(
	...
	describe.skipIf(!process.env.OPENAI_APHI_KEY)("OpenAI Responses Provider (gpt-5.4)", () => {
	...
	describe.skipIf(!process.env.XAI_APHI_KEY)("xAI Provider (grok-4.3 via OpenAI Responses)", () => {
	...
	describe.skipIf(!process.env.GROQ_APHI_KEY)("Groq Provider (gpt-oss-20b via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.CEREBRAS_APHI_KEY)("Cerebras Provider (gpt-oss-120b via OpenAI Completions)", () => {
	...
	describe.skipIf(!hasCloudflareAiGatewayCredentials() || !process.env.OPENAI_APHI_KEY)(
		"Cloudflare AI Gateway → OpenAI BYOK (gpt-5.1 via /openai responses)",
		() => {
			const llm = getModel("cloudflare-ai-gateway", "gpt-5.1");
			const options = { headers: { Authorization: `Bearer ${process.env.OPENAI_APHI_KEY}` } };
	...
	describe.skipIf(!hasCloudflareAiGatewayCredentials() || !process.env.ANTHROPIC_APHI_KEY)(
		...
		() => {
			...
			const options = { headers: { Authorization: `Bearer ${process.env.ANTHROPIC_APHI_KEY}` } };
			const thinkingOptions = {
				...options,
				thinkingEnabled: true,
	...
	describe.skipIf(!process.env.TOGETHER_APHI_KEY)("Together AI Provider (Kimi-K2.6 via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.BASETEN_APHI_KEY)("Baseten Provider (GLM 5.2 via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.NVIDIA_APHI_KEY)(
	...
	describe.skipIf(!process.env.OPENROUTER_APHI_KEY)("OpenRouter Provider (glm-4.5v via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.AI_GATEWAY_APHI_KEY)(
	...
	describe.skipIf(!process.env.ZAI_APHI_KEY)("zAI Provider (glm-5.2 via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.MISTRAL_APHI_KEY)("Mistral Provider (devstral-medium-latest)", () => {
	...
	describe.skipIf(!process.env.MISTRAL_APHI_KEY)("Mistral Provider (pixtral-12b with image support)", () => {
	...
	describe.skipIf(!process.env.MINIMAX_APHI_KEY)("MiniMax Provider (MiniMax-M2.7 via Anthropic Messages)", () => {
	...
	describe.skipIf(!process.env.KIMI_APHI_KEY)(
	...
	describe.skipIf(!process.env.XIAOMI_APHI_KEY)(
	...
	describe.skipIf(!process.env.QWEN_TOKEN_PLAN_APHI_KEY)(
	...
	describe.skipIf(!process.env.QWEN_TOKEN_PLAN_CN_APHI_KEY)(
	...
	describe.skipIf(!process.env.ANT_LING_APHI_KEY)("Ant Ling Provider (Ling 2.6 Flash via OpenAI Completions)", () => {
