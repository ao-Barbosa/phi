# @ao-Barbosa/phi-client

Transport-neutral client for the experimental Phi service protocol.

```ts
import { Client, type ByteTransportFactory } from "@ao-Barbosa/phi-client";
...
```ts
import { Client } from "@ao-Barbosa/phi-client";
import { createUnixTransportFactory } from "@ao-Barbosa/phi-client/unix";

const client = new Client({
  serverId: "01234567-89ab-4def-8123-456789abcdef",
  transportFactory: createUnixTransportFactory({ path: "/tmp/phi.sock" }),
});
...
```ts
import { discoverUnixServers } from "@ao-Barbosa/phi-client/unix";

const routes = await discoverUnixServers({ directory: "/run/user/1000/phi" });
// [{ serverId: "...", path: "/run/user/1000/phi/<serverId>.sock" }]
```
