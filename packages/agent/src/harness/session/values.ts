import type { AssistantMessageFrame } from "@ao-barbosa/phi-ai";
...
export const branchTip = (branch: string) => value<string | null>("phi.branch.tip", branch);
export const branchTipInventoryPrefix = () => value<string | null>("phi.branch.tip");
export const laneConfig = (lane: string) => value<LaneConfiguration>("phi.lane.config", lane);
export const laneState = (lane: string) => value<LaneState>("phi.lane.state", lane);
export const operationResult = (operationId: string) => value<OperationResultRecord>("phi.result", operationId);

export const operationMeta = (operationId: string) => value<OperationMeta>("phi.op.meta", operationId);
export const operationState = (operationId: string) => value<OperationState>("phi.op.state", operationId);
export const operationToolArgs = (operationId: string, stepId: string, sourceIndex: number) =>
	value<Record<string, JsonValue>>("phi.op.tool_args", `${operationId}:${stepId}:${sourceIndex}`);
...
export const operationPreparation = (operationId: string, taskId: string) =>
	value<DurableStructuralPreparation>("phi.op.preparation", `${operationId}:${taskId}`);
...
export const pendingEntry = (entryId: string) => value<PendingEntry>("phi.pending.entry", entryId);
export const pendingToolOutput = (operationId: string, invocationId: string) =>
	value<AgentToolResult<unknown>>("phi.pending.tool_output", `${operationId}:${invocationId}`);
export const pendingAssistantFrames = (operationId: string, responseEntryId: string) =>
	list<AssistantMessageFrame>("phi.pending.assistant_frame", `${operationId}:${responseEntryId}`);
