...
describe("runtime structural drive", () => {
	...
	it("gives each split-turn provider request its own durable intent and usage row", async () => {
		...
		expect(
			attempts.flat().some((write) => write.kind === "list" && write.namespace === "phi.pending.assistant_frame"),
		).toBe(false);
		expect(
	...
	it("moves an unsummarized navigation and cleans up in one terminal transaction", async () => {
		...
		expect(
			writes.some((write) => write.kind === "value" && write.namespace === "phi.op.state" && write.op === "delete"),
		).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "phi.result")).toBe(true);
		expect(writes.some((write) => write.kind === "value" && write.namespace === "phi.lane.state")).toBe(true);
	});
