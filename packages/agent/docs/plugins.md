...
`@ao-barbosa/phi-chord`. This document specifies how Phi composes that runtime with Phi-owned
...
→ @phi/providers-builtin:session provides phi.models/singleton

env.use(Models)
→ @phi/model-selection:tui requires phi.models/singleton

env.provideMany(QuestionDialogs)
→ @phi/question:session provides phi.question-dialog/keyed

env.observe(QuestionDialogs, handler)
→ @phi/question:tui requires phi.question-dialog/keyed
```
...
const Agent = defineService<AgentFacetScope>("phi.local.agent", { local: true });
const Providers = defineService<ProviderContributionRegistry>("phi.local.providers", { local: true });
const Tools = defineService<ToolContributionRegistry>("phi.local.tools", { local: true });
```
...
const Tui = defineService<TuiHost>("phi.local.tui", { local: true });
```
...
export function createAgentControllerRuntimeFacet(lane: AgentLane) {
	return defineFacet({
		id: "@phi/agent-controller-runtime",
		setup(env) {
...
const Accounts = defineService<Accounts>("phi.accounts");
```
...
```text
fresh ProviderDraft
→ built-in provider contribution        (@phi/providers-builtin)
→ remote catalogue contribution         (@phi/providers-catalog)
→ models.json transformation            (@phi/providers-models-json)
→ authentication/availability marking   (@phi/auth)
...
const Fleet = defineService<FleetFacetScope>("phi.local.fleet", { local: true });
```
...
const QuestionDialogs = defineService<QuestionDialogs>("phi.question-dialog");
```
		...
		tools.add((draft) => {
			draft.set("question", {
				...
				async execute(_toolCallId, params, _onUpdate, _toolContext, invocation, context) {
					...
					const memoName = "phi.question.answer";
...
const DiffReviewManager = defineService<DiffReviewManager>("phi.diff-review-manager");
const DiffReviews = defineService<DiffReviews>("phi.diff-review");
```
