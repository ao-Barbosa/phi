...
/** Stored at operationResult(operationId) — namespace "phi.result" — by the terminal transaction. Immutable, lane-lived. */
...
- `src/harness/session/values.ts` — `operationResult(operationId) = value<OperationResultRecord>("phi.result", operationId)`; delete `laneLastResult`. The `phi.op.*` lifetime grammar stays total: no operation-lived namespace survives its terminal transaction.
...
- Forks exclude `phi.result` values.
...
- §1.3 address table and lifetime grammar: new lane-lived `phi.result` namespace, `laneLastResult` removed, `LaneState.inbox`/`lastOperationId`; `phi.op.*` stays strictly operation-lived;
- §1.7 JSONL/SQLite examples referencing `phi.lane.lastResult`; state the bounded growth tradeoff — one small immutable record per operation, retained forever, carried through JSONL snapshot compaction;
...
**Tracked follow-up; not part of the M8/M10 public-drive gate.** A trivial mini coding-agent Session produced an approximately 300 KB JSONL file because live assistant streaming persists many `phi.pending.assistant_frame` list appends. Logical frame cleanup does not reclaim bytes already appended to the JSONL history, so short conversations can have disproportionate durable storage and replay cost.
