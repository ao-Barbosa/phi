...
await session.readList(workspaceEvents("phi"), { limit: 100 }, context);
```
...
- namespace `phi` and every `phi.*` namespace are reserved for built-ins by contract;
...
export const branchTip = (lane: string) =>
  value<string | null>("phi.branch.tip", lane);

export const operationState = (operationId: string) =>
  value<OperationState>("phi.op.state", operationId);
...
export const operationToolMemo = (operationId: string, invocationId: string, name: string) =>
  value<JsonValue>("phi.op.tool_memo", `${operationId}:${invocationId}:${name}`);
...
export const pendingToolOutputPrefix = (operationId: string) =>
  value<AgentToolResult<unknown>>("phi.pending.tool_output", `${operationId}:`);

export const sessionName = value<string>("phi.session.name");
export const entryLabel = (entryId: string) => value<string>("phi.entry.label", entryId);
```

`OperationMeta` is immutable acceptance metadata stored at `phi.op.meta`. The process-local `Operation` projection is `{ meta: OperationMeta, state: OperationState }`, assembled from the separate metadata and state values; it is never stored at one address.
...
`AssistantMessageFrame`, `AssistantMessageFrameEncoder`, and `reduceAssistantMessageFrames()` come from `@ao-Barbosa/phi-ai`. Do not define a second frame codec or reducer.
...
```jsonl
{"kind":"list","op":"append","seq":41,"namespace":"phi.pending.assistant_frame","key":"O:R","value":{"type":"text_delta","contentIndex":0,"delta":"hi"}}
{"kind":"list","op":"delete","seq":52,"namespace":"phi.pending.assistant_frame","key":"O:R"}
```
...
- operation-owned `phi.op.*` scalar values are not copied into an idle fork;
- immutable `phi.result` operation records are not copied by forks;
- `phi.pending.entry`, `phi.pending.tool_output`, and `phi.pending.assistant_frame` values/lists are not copied;
...
3. Namespace `phi` and every `phi.*` are reserved by contract; every built-in namespace starts with `phi.`, and application use is a trusted-programming defect.
...
- built-in address constructors produce exact `phi.branch.tip`, `phi.lane.*`, `phi.op.*`, `phi.pending.*`, `phi.session.name`, and `phi.entry.label` namespace/key/kind triples;
- every built-in namespace starts with `phi.`, while application fixtures use non-reserved namespaces;
- `branchTipInventoryPrefix()` binds the empty-key `phi.branch.tip` inventory prefix and is used only to enumerate Branches through `scanValues`;
