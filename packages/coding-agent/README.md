...
Phi is a minimal terminal coding harness. Adapt phi to your workflows, not the other way around, without having to fork and modify phi internals. Extend it with TypeScript [Extensions](#extensions), [Skills](#skills), [Prompt Templates](#prompt-templates), and [Themes](#themes). Put your extensions, skills, prompt templates, and themes in [Phi Packages](#phi-packages) and share them with others via npm or git.

Phi ships with powerful defaults but skips features like sub agents and plan mode. Instead, you can ask phi to build what you want or install a third party phi package that matches your workflow.

Phi runs in four modes: interactive, print or JSON, RPC for process integration, and an SDK for embedding in your own apps.

## Share your OSS coding agent sessions

If you use phi for open source work, please share your coding agent sessions.
...
- **Update check:** fetches `https://phi.dev/api/latest-version` to check whether a newer Phi version exists. Disable it with `PHI_SKIP_VERSION_CHECK=1`. Disabling update checks only turns off this check.
- **Install/update telemetry:** after first install or a changelog-detected update, sends an anonymous version ping to `https://phi.dev/api/report-install`. This setting also controls optional provider attribution headers for OpenRouter, Cloudflare, and direct NVIDIA NIM requests. Opt out by setting `enableInstallTelemetry` to `false` in `settings.json`, or by setting `PHI_TELEMETRY=0`. This does not disable update checks; Phi may still contact `phi.dev` for the latest version unless update checks are disabled or offline mode is enabled.

Use `--offline` or `PHI_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
export default function (phi: ExtensionAPI) {
  phi.registerTool({ name: "deploy", ... });
  phi.registerCommand("stats", { ... });
  phi.on("tool_call", async (event, ctx) => { ... });
}
```

The default export can also be `async`. phi waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `phi.registerProvider()`.
...
```bash
phi install npm:@foo/phi-tools
phi install npm:@foo/phi-tools@1.2.3      # pinned version
...
phi remove npm:@foo/phi-tools
phi uninstall npm:@foo/phi-tools          # alias for remove
phi list
...
phi update npm:@foo/phi-tools             # update one package
...
Create a package by adding a `phi` key to `package.json`:

```json
{
  "name": "my-phi-package",
  "keywords": ["phi-package"],
  "phi": {
    "extensions": ["./extensions"],
    "skills": ["./skills"],
    "prompts": ["./prompts"],
...
Without a `phi` manifest, phi auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
```typescript
import { createAgentSession, ModelRuntime, SessionManager } from "@ao-Barbosa/phi-coding-agent";
...
Read the [blog post](https://mariozechner.at/posts/2025-11-30-phi-coding-agent/) for the full rationale.
...
|----------|-------------|
| `AI_AGENT` | Set to `phi` by the CLI and RPC entry points so generic tooling can attribute child processes to Phi |
| `PHI_CODING_AGENT` | Set to `true` by the CLI and RPC entry points so child processes can detect that they run inside Phi |
...
| `PHI_PACKAGE_DIR` | Override package directory (useful for Nix/Guix where store paths tokenize poorly) |
| `PHI_SERVER_DIR` | Override experimental server profile and socket directory (default: `~/.phi/server`) |
| `PHI_SERVER_ID` | Select the logical experimental server ID when `--server-id` is omitted |
| `PHI_OFFLINE` | Disable startup network operations, including update checks, package update checks, and install/update telemetry |
| `PHI_SKIP_VERSION_CHECK` | Skip the Phi version update check at startup. This prevents the `phi.dev` latest-version request |
| `PHI_TELEMETRY` | Override install/update telemetry and provider attribution headers. Use `1`/`true`/`yes` to enable or `0`/`false`/`no` to disable. This does not disable update checks |
| `PHI_CACHE_RETENTION` | Set to `long` for extended prompt cache (Anthropic: 1h, OpenAI: 24h) |
...
| `PHI_SESSION_FILE` | Absolute session JSONL path; unset for ephemeral sessions |
| `PHI_PROVIDER` | Currently selected model provider |
| `PHI_MODEL` | Currently selected model ID |
| `PHI_REASONING_LEVEL` | Current effective reasoning level |
...
- [@ao-Barbosa/phi-ai](https://www.npmjs.com/package/@ao-Barbosa/phi-ai): Core LLM toolkit
- [@ao-Barbosa/phi-agent-core](https://www.npmjs.com/package/@ao-Barbosa/phi-agent-core): Agent framework
- [@ao-Barbosa/phi-tui](https://www.npmjs.com/package/@ao-Barbosa/phi-tui): Terminal UI components
