...
Extensions can register custom model providers via `phi.registerProvider()`. This enables:
...
Extensions can register either a complete phi-ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. Phi composes `models.json` overrides above registered native providers.
...
export default function (phi: ExtensionAPI) {
  phi.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  phi.registerProvider("my-provider", {
    name: "My Provider",
...
```typescript
phi.registerProvider("my-llm", {
  baseUrl: "https://api.my-llm.com/v1",
  apiKey: "$MY_LLM_APHI_KEY",  // env var reference
...
Use `phi.unregisterProvider(name)` to remove a provider that was previously registered via `phi.registerProvider(name, ...)`:

```typescript
// Register
phi.registerProvider("my-llm", {
  baseUrl: "https://api.my-llm.com/v1",
  apiKey: "$MY_LLM_APHI_KEY",
  api: "openai-completions",
  models: [
    {
      id: "my-llm-large",
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map phi levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
phi.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_APHI_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@ao-Barbosa/phi-ai";
...
- [anthropic-messages.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/anthropic-messages.ts) - Anthropic Messages API
- [mistral-conversations.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/mistral-conversations.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/openai-responses.ts) - OpenAI Responses API
- [google-generative-ai.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/google-generative-ai.ts) - Google Generative AI
- [bedrock-converse-stream.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/api/bedrock-converse-stream.ts) - AWS Bedrock
...
When a request exceeds the model's context window, phi can recover automatically by compacting the conversation and retrying. This recovery only kicks in if phi recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of phi's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message phi does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase phi recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before phi tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what phi checks. With this in place, phi will:
...
- Match a provider-specific pattern, not phi's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of phi's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/ao-Barbosa/phi/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps phi thinking levels to provider/model-specific values; null marks a level unsupported. */
