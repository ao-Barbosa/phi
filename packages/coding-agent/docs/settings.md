...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`PHI_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so phi resumes after the editor exits:
...
`enableInstallTelemetry` controls the anonymous install/update ping to `https://phi.dev/api/report-install` and Phi attribution headers for OpenRouter, NVIDIA NIM, and Cloudflare provider requests. Opting out disables both. It does not disable update checks; Phi can still fetch `https://phi.dev/api/latest-version` to look for the latest version.

Set `PHI_SKIP_VERSION_CHECK=1` to disable the Phi version update check. Use `--offline` or `PHI_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before Phi sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.phi/agent/npm/`; project-scoped npm packages install under `.phi/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.

### Tools

| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `defaultTools` | string[] | - | Built-in tools enabled initially. When omitted, Phi uses its standard defaults |
...
Paths in `~/.phi/agent/settings.json` resolve relative to `~/.phi/agent`. Paths in `.phi/settings.json` resolve relative to `.phi`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["phi-skills", "@org/my-extension"]
}
```
