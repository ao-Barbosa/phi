...
**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `phi.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `phi.registerCommand()`
- **Session persistence** - Store state that survives restarts via `phi.appendEntry()`
...
export default function (phi: ExtensionAPI) {
  // React to events
  phi.on("session_start", async (_event, ctx) => {
    ctx.ui.notify("Extension loaded!", "info");
  });

  phi.on("tool_call", async (event, ctx) => {
...
Extensions are auto-discovered from trusted locations. Project-local `.phi/extensions` entries load only after the project is trusted.
...
| Package | Purpose |
|---------|---------|
| `@ao-Barbosa/phi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@ao-Barbosa/phi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@ao-Barbosa/phi-tui` | TUI components for custom rendering |
...
export default function (phi: ExtensionAPI) {
  // Subscribe to events
  phi.on("event_name", async (event, ctx) => {
  ...
  phi.registerTool({ ... });
  phi.registerCommand("name", { ... });
  phi.registerShortcut("ctrl+x", { ... });
  phi.registerFlag("my-flag", { ... });
}
```

Extensions are loaded via [jiti](https://github.com/unjs/jiti), so TypeScript works without compilation.

If the factory returns a `Promise`, phi awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `phi.registerProvider()` are flushed.
...
export default async function (phi: ExtensionAPI) {
  ...
  phi.registerProvider("local-openai", {
    baseUrl: "http://localhost:1234/v1",
    apiKey: "$LOCAL_OPENAI_APHI_KEY",
...
thinking level changes (settings, keybinding, phi.setThinkingLevel())
...
Fired before phi decides whether to trust a project with dynamic configs (`.phi` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.

```typescript
phi.on("project_trust", async (event, ctx) => {
...
A `project_trust` handler must return `{ trusted: "yes" | "no" | "undecided" }`. A user/global or CLI extension that returns `"yes"` or `"no"` owns the decision; the first yes/no decision wins and suppresses the built-in trust prompt. Use `remember: true` to persist a yes/no decision; otherwise it applies only to the current process. Return `"undecided"` to let later handlers or the built-in trust flow decide. Check `ctx.hasUI` before prompting. If no handler returns yes/no, normal trust resolution continues: saved `trust.json` decisions apply first, then `defaultProjectTrust` controls whether phi asks, trusts, or declines by default.
...
```typescript
phi.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
```typescript
phi.on("session_start", async (event, ctx) => {
...
Fired when the current session display name is set via `/name`, RPC, or `phi.setSessionName()`.

```typescript
phi.on("session_info_changed", async (event, ctx) => {
...
```typescript
phi.on("session_before_switch", async (event, ctx) => {
...
```typescript
phi.on("session_before_fork", async (event, ctx) => {
...
```typescript
phi.on("session_before_compact", async (event, ctx) => {
...
phi.on("session_compact", async (event, ctx) => {
...
phi.on("session_compact_failed", async (event, ctx) => {
  // event.reason - "manual" (/compact), "threshold", or "overflow"
...
```typescript
phi.on("session_before_tree", async (event, ctx) => {
...
phi.on("session_tree", async (event, ctx) => {
...
```typescript
phi.on("session_shutdown", async (event, ctx) => {
...
```typescript
phi.on("before_agent_start", async (event, ctx) => {
...
`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but Phi may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know Phi will not continue running automatically.

```typescript
phi.on("agent_start", async (_event, ctx) => {});

phi.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

phi.on("agent_settled", async (_event, ctx) => {
...
```typescript
phi.on("ui_prompt_start", async (event, ctx) => {
  // event.reason === "ui_prompt"
  // event.kind: "select" | "confirm" | "input" | "editor" | "custom"
  // event.title: prompt title when available
});

phi.on("ui_prompt_end", async (event, ctx) => {
  // Phi is no longer waiting on that UI prompt span.
});
```
...
```typescript
phi.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

phi.on("turn_end", async (event, ctx) => {
...
```typescript
phi.on("message_start", async (event, ctx) => {
  // event.message
});

phi.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

phi.on("message_end", async (event, ctx) => {
...
```typescript
phi.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

phi.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

phi.on("tool_execution_end", async (event, ctx) => {
...
```typescript
phi.on("context", async (event, ctx) => {
...
```typescript
phi.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header phi adds for this call
...
```typescript
phi.on("before_provider_request", (event, ctx) => {
...
```typescript
phi.on("after_provider_response", (event, ctx) => {
...
```typescript
phi.on("model_select", async (event, ctx) => {
...
```typescript
phi.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `phi.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
```typescript
import { isToolCallEventType } from "@ao-Barbosa/phi-coding-agent";
...
```typescript
import { isBashToolResult } from "@ao-Barbosa/phi-coding-agent";

phi.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "@ao-Barbosa/phi-coding-agent";

phi.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap phi's built-in local bash backend
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.phi` when constructing project-local config paths. Rebranded distributions can use a different config directory name.

```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@ao-Barbosa/phi-coding-agent";
import { join } from "node:path";

export default function (phi: ExtensionAPI) {
  phi.on("session_start", (_event, ctx) => {
...
Access to models, providers, and resolved authentication. `ctx.modelRegistry.getProvider(id)` returns the effective phi-ai provider, while `getProviderAuth(id)` resolves its current API key, headers, base URL, and provider-scoped environment without requiring a loaded model. `ctx.model` is the active model, and `ctx.thinkingLevel` is its current effective thinking level.
...
Control flow helpers. `ctx.isIdle()` is false while Phi is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of phi.
...
```typescript
phi.on("before_agent_start", (event, ctx) => {
...
Returns the base inputs Phi currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `phi` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@ao-Barbosa/phi-coding-agent";

phi.registerCommand("switch", {
...
- Captured old `phi` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
phi.registerCommand("reload-runtime", {
...
export default function (phi: ExtensionAPI) {
  ...
  phi.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      phi.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
### phi.on(event, handler)

Subscribe to events. See [Events](#events) for event types and return values.

### phi.registerTool(definition)

Register a custom tool callable by the LLM. See [Custom Tools](#custom-tools) for full details.

`phi.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `phi.getAllTools()` and are callable by the LLM without `/reload`.

Use `phi.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@ao-Barbosa/phi-ai";
...
### phi.sendMessage(message, options?)

Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`phi.appendEntry()`](#piappendentrycustomtype-data) with [`phi.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### phi.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
phi.sendUserMessage("What is 2+2?");
...
phi.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
phi.sendUserMessage("And then summarize", { deliverAs: "followUp" });

// Opt in to extension command dispatch and skill/prompt template expansion
phi.sendUserMessage("/review src/index.ts", { expandPromptTemplates: true });
```
...
### phi.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `phi.registerEntryRenderer()`.

```typescript
phi.appendEntry("my-state", { count: 42 });
phi.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
### phi.setSessionName(name)

Set the session display name (shown in session selector instead of first message).

```typescript
phi.setSessionName("Refactor auth module");
```
...
```typescript
const name = phi.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### phi.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
phi.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
phi.setLabel(entryId, undefined);
...
### phi.registerCommand(name, options)
...
```typescript
import type { AutocompleteItem } from "@ao-Barbosa/phi-tui";

phi.registerCommand("deploy", {
...
```typescript
const commands = phi.getCommands();
...
### phi.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `phi.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### phi.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, Phi renders the transformed content with its built-in renderer.
...
```typescript
phi.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
...
### phi.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `phi.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@ao-Barbosa/phi-tui";

phi.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### phi.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
phi.registerShortcut("ctrl+shift+p", {
...
### phi.registerFlag(name, options)

Register a CLI flag.

```typescript
phi.registerFlag("plan", {
...
### phi.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await phi.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### phi.getActiveTools() / phi.getAllTools() / phi.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `phi.getActiveTools()` returns the active tool names as `string[]`; `phi.getAllTools()` returns metadata for all configured tools.

```typescript
const active = phi.getActiveTools(); // ["read", "bash", ...]
const all = phi.getAllTools();
// all = [{
//   name: "read",
...
phi.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
phi.setActiveTools(["read", "bash"]); // Switch to read-only
```

`phi.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await phi.setModel(model);
  if (!success) {
...
### phi.getThinkingLevel() / phi.setThinkingLevel(level)
...
`phi.setThinkingLevel()` changes the thinking level for the current session. The change is recorded in session history and restored when that session is resumed, but it does not change the configured default used by new sessions.

```typescript
const current = phi.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
phi.setThinkingLevel("high");
```
...
```typescript
phi.events.on("my:event", (data) => { ... });
phi.events.emit("my:event", { ... });
```

### phi.registerProvider(name, config)
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@ao-Barbosa/phi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@ao-Barbosa/phi-ai";
...
phi.registerProvider(provider);

// Register a new provider with custom models
phi.registerProvider("my-proxy", {
  name: "My Proxy",
  baseUrl: "https://proxy.example.com",
  apiKey: "$PROXY_APHI_KEY",  // env var reference
...
phi.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
phi.registerProvider("anthropic", {
  baseUrl: "https://proxy.example.com"
});

// Register provider with OAuth support for /login
phi.registerProvider("corporate-ai", {
...
The object form accepts a complete phi-ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
phi.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    phi.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `phi.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `phi.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@ao-Barbosa/phi-coding-agent";
...
phi.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via phi.exec (captured from extension closure)
    const result = await phi.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. Phi persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@ao-Barbosa/phi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [powershell.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/powershell.ts) - `PowerShellToolDetails`
- [edit.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@ao-Barbosa/phi-coding-agent";
...
```typescript
import { createBashTool } from "@ao-Barbosa/phi-coding-agent";
...
`createBashTool()` and `createPowerShellTool()` expose the current session to commands through `PHI_SESSION_ID`, `PHI_SESSION_FILE`, `PHI_PROVIDER`, `PHI_MODEL`, and `PHI_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (phi: ExtensionAPI) {
  ...
  phi.registerTool({ name: "db_connect", ... });
  phi.registerTool({ name: "db_query", ... });
  phi.registerTool({ name: "db_close", ... });

  phi.on("session_shutdown", async () => {
    connection?.close();
  });
}
```
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@ao-Barbosa/phi-coding-agent";
...
1. Register every tool with `phi.registerTool()` so it appears in `phi.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `phi.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. Phi records which tools were added on the loader's tool result.
5. Before the next model response, Phi exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `phi.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** Phi adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: Phi sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

Phi also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (phi: ExtensionAPI) {
  ...
  phi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const active = phi.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      phi.setActiveTools([...new Set([...active, ...added])]);
  ...
  phi.on("session_start", () => {
    // Keep searchable tools registered but initially inactive. Preserve built-ins
    // and tools owned by other extensions, and keep the loader itself active.
    const initialTools = phi.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    phi.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
// Terminal title
ctx.ui.setTitle("phi - my-project");
...
```typescript
import { Text, Component } from "@ao-Barbosa/phi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@ao-Barbosa/phi-coding-agent";
import { matchesKey } from "@ao-Barbosa/phi-tui";
...
phi.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `phi.sendMessage()`:
...
```typescript
phi.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

phi.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "@ao-Barbosa/phi-coding-agent";
...
| `event-bus.ts` | Inter-extension events | `phi.events` |
| **Session Metadata** |||
