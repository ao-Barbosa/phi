...
```bash
npm install @ao-Barbosa/phi-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@ao-Barbosa/phi-coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `phi.sendMessage()`.
...
The `Agent` class (from `@ao-Barbosa/phi-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
```typescript
import { getModel } from "@ao-Barbosa/phi-ai";
import { ModelRuntime } from "@ao-Barbosa/phi-coding-agent";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from phi.dev by default.
...
3. Environment variables (`ANTHROPIC_APHI_KEY`, `OPENAI_APHI_KEY`, etc.)
4. Fallback resolver (for custom provider keys from `models.json`)

```typescript
import { InMemoryCredentialStore } from "@ao-Barbosa/phi-ai";
import { createAgentSession, ModelRuntime } from "@ao-Barbosa/phi-coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@ao-Barbosa/phi-coding-agent";
...
```typescript
import { createAgentSession } from "@ao-Barbosa/phi-coding-agent";
...
import { createAgentSession, defineTool } from "@ao-Barbosa/phi-coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `phi.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `phi.registerTool()`.
...
```typescript
import type { InlineExtension } from "@ao-Barbosa/phi-coding-agent";
...
**Event Bus:** Extensions can communicate via `phi.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@ao-Barbosa/phi-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@ao-Barbosa/phi-coding-agent";
