...
Phi packages bundle extensions, skills, prompt templates, and themes so you can share them through npm or git. A package can declare resources in `package.json` under the `phi` key, or use conventional directories.

## Table of Contents

- [Install and Manage](#install-and-manage)
- [Package Sources](#package-sources)
- [Creating a Phi Package](#creating-a-phi-package)
...
These commands manage phi packages and `phi update` can update the phi CLI installation. For experimental installer-managed installations, `phi update` installs the exact checked version into a staged, lockfile-backed release and activates it only after verification, leaving the current release intact if the update fails. Managed installations do not support `--force`; rerun the installer to repair one. To uninstall phi itself, see [Quickstart](quickstart.md#uninstall).
...
Add a `phi` manifest to `package.json` or use conventional directories. Include the `phi-package` keyword for discoverability.
...
The [package gallery](https://phi.dev/packages) displays packages tagged with `phi-package`. Add `video` or `image` fields to show a preview:
...
If no `phi` manifest is present, phi auto-discovers resources from these directories:
...
Phi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@ao-Barbosa/phi-ai`, `@ao-Barbosa/phi-agent-core`, `@ao-Barbosa/phi-coding-agent`, `@ao-Barbosa/phi-tui`, `typebox`.
