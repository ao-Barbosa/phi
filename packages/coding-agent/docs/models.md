...
```json
{
  "providers": {
    "custom-proxy": {
      ...
      "headers": {
        "x-portkey-api-key": "$PORTKEY_APHI_KEY",
...
| `supportsMidConvoEffort` | Whether the exact Claude model transport supports per-turn effort system messages and thinking binding controls. Phi persists native effort levels and always sends `drop_block` when enabled. Default: `false`. |
...
| `chatTemplateKwargs` | `chat_template_kwargs` values for `thinkingFormat: "chat-template"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for phi-controlled thinking values |
| `chatTemplateArgs` | `chat_template_args` values for `thinkingFormat: "baseten"`; use `{ "$var": "thinking.enabled" }`, `{ "$var": "thinking.effort" }`, or `{ "$var": "thinking.budget" }` for phi-controlled thinking values |
