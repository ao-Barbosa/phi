...
Source on GitHub ([phi](https://github.com/ao-Barbosa/phi)):
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/session-manager.ts) - Session entry types and SessionManager
- [`packages/coding-agent/src/core/messages.ts`](https://github.com/ao-Barbosa/phi/blob/main/packages/coding-agent/src/core/messages.ts) - Extended message types (BashExecutionMessage, CustomMessage, etc.)
- [`packages/ai/src/types.ts`](https://github.com/ao-Barbosa/phi/blob/main/packages/ai/src/types.ts) - Base message types (UserMessage, AssistantMessage, ToolResultMessage)
- [`packages/agent/src/types.ts`](https://github.com/ao-Barbosa/phi/blob/main/packages/agent/src/types.ts) - AgentMessage union type

For TypeScript definitions in your project, inspect `node_modules/@ao-Barbosa/phi-coding-agent/dist/` and `node_modules/@ao-Barbosa/phi-ai/dist/`.
...
### Base Message Types (from phi-ai)
...
The exported phi-ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before phi persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from phi-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if phi-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `phi.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `phi.setSessionName()` in extensions.
