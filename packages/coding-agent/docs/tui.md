...
**Source:** [`@ao-Barbosa/phi-tui`](https://github.com/ao-Barbosa/phi/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@ao-Barbosa/phi-tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer's `showHardwareCursor` constructor argument or `setShowHardwareCursor(true)`. Phi also maps `PHI_HARDWARE_CURSOR=1` to this setting before it creates its renderer. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@ao-Barbosa/phi-tui";
...
Import from `@ao-Barbosa/phi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@ao-Barbosa/phi-tui";
```
...
```typescript
import { matchesKey, Key } from "@ao-Barbosa/phi-tui";
...
```typescript
import { MouseRegion } from "@ao-Barbosa/phi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@ao-Barbosa/phi-tui";
...
```typescript
import { getMarkdownTheme } from "@ao-Barbosa/phi-coding-agent";
import { Markdown } from "@ao-Barbosa/phi-tui";
...
Set `PHI_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
PHI_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@ao-Barbosa/phi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@ao-Barbosa/phi-coding-agent";
import { DynamicBorder } from "@ao-Barbosa/phi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@ao-Barbosa/phi-tui";
...
```typescript
import { BorderedLoader } from "@ao-Barbosa/phi-coding-agent";

phi.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@ao-Barbosa/phi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@ao-Barbosa/phi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@ao-Barbosa/phi-tui";

phi.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
import { matchesKey, truncateToWidth } from "@ao-Barbosa/phi-tui";
