...
Example extensions for phi-coding-agent.

## Usage

```bash
# Load an extension with --extension flag
phi --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.phi/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `phi.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs phi theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `phi.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using phi-ai's built-in Anthropic/OpenAI streaming via proxy |
