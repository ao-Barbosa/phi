...
The package needs no build script. Phi asks Chord to discover `src/session.ts` and `src/tui.ts`, builds both entries into its server-owned plugin cache, and sends the TUI artifact to clients.

From the repository root:

```bash
PHI_EXPERIMENTAL=1 ./phi-test.sh server \
  -e "$PWD/packages/coding-agent/examples/plugins/phi-example-plugin"
```

Alternatively, a client can select the plugin for the Session it creates or resumes on one local server:

```bash
PHI_EXPERIMENTAL=1 ./phi-test.sh client \
  -e "$PWD/packages/coding-agent/examples/plugins/phi-example-plugin"
```
