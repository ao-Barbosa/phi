...
const originalPiPackageDir = process.env.PHI_PACKAGE_DIR;
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@ao-barbosa", "phi-coding-agent");
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@ao-barbosa+phi-coding-agent@0.0.0",
			"node_modules",
			"@ao-barbosa",
			"phi-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@ao-barbosa", "phi-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.PHI_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@ao-barbosa", "phi-coding-agent", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		setExecPath(
			"C:\\Users\\Admin\\Documents\\pnpm-repository\\global\\5\\.pnpm\\@ao-barbosa+phi-coding-agent@0.67.68\\node_modules\\@ao-barbosa\\phi-coding-agent\\dist\\cli.js",
		);
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@ao-barbosa/phi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @ao-barbosa/phi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.PHI_PACKAGE_DIR = storePackageDir;
