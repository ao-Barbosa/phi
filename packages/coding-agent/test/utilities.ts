...
import type { OAuthCredentials } from "@ao-barbosa/phi-ai";
import { getModel, streamSimple } from "@ao-barbosa/phi-ai/compat";
import { builtinProviders } from "@ao-barbosa/phi-ai/providers/all";
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `phi-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
