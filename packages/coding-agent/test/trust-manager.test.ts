...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".phi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".phi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".phi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".phi"), { recursive: true });
			writeFileSync(join(cwd, ".phi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".phi"), { recursive: true, force: true });
