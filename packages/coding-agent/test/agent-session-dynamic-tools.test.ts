...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `phi-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(session.systemPrompt).toContain(
			"You can inspect PHI_* environment variables for current model and session details.",
		);
		await bashTool.execute("bash-env", { command: "printf ok" });
		expect(sessionEnv).toMatchObject({
			PHI_SESSION_ID: session.sessionId,
			PHI_SESSION_FILE: session.sessionFile,
			PHI_PROVIDER: model.provider,
			PHI_MODEL: model.id,
			PHI_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("PHI_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("PHI_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("PHI_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("PHI_MODEL");
		expect(optedOutEnv).not.toHaveProperty("PHI_REASONING_LEVEL");
