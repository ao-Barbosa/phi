...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "phi-ext-test-"));
	...
	it("loads the coding-agent entrypoint without rewriting pi-ai provider subpaths", async () => {
		fs.writeFileSync(
			...
			`
				import { getAgentDir } from "@ao-barbosa/phi-coding-agent";
				void getAgentDir;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only pi-ai OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			...
			`
				import * as oauth from "@ao-barbosa/phi-ai/oauth";
				void oauth;
				export default function(pi) {
					pi.registerCommand("test", { handler: async () => {} });
				}
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ phi: { extensions: ["./entry.ts"] } }));
