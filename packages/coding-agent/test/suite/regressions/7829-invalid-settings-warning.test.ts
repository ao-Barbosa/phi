...
import { beforeAll, describe, expect, it, vi } from "../../../../../test-support/vi.ts";
...
describe("issue #7829 invalid settings warning", () => {
	...
	it("renders startup diagnostics inside the transcript", async () => {
		...
		const previousOffline = process.env.PHI_OFFLINE;
		...
		try {
			...
			if (previousOffline === undefined) delete process.env.PHI_OFFLINE;
			else process.env.PHI_OFFLINE = previousOffline;
			harness.cleanup();
		}
	});
});
