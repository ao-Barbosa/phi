...
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "@ao-barbosa/phi-ai/compat";
import { registerFauxProvider, streamSimple } from "@ao-barbosa/phi-ai/compat";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `phi-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
