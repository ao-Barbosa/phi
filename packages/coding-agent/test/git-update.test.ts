...
describe("DefaultPackageManager git update", () => {
	...
	describe("temporary git sources", () => {
		it("should refresh cached temporary git sources when resolving", async () => {
			...
			const extensionFile = join(cachedDir, "phi-extensions", "session-breakdown.ts");

			rmSync(cachedDir, { recursive: true, force: true });
			mkdirSync(join(cachedDir, "phi-extensions"), { recursive: true });
			writeFileSync(
				join(cachedDir, "package.json"),
				JSON.stringify({ phi: { extensions: ["./phi-extensions"] } }, null, 2),
			);
			...
			expect(getFileContent(cachedDir, "phi-extensions/session-breakdown.ts")).toBe("// fresh");
		});
	});
});
