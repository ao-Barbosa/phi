 ...
 * RPC mode tests.
 */
describe.skipIf(!process.env.ANTHROPIC_APHI_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	let client: RpcClient;
	let sessionDir: string;

	beforeEach(() => {
		sessionDir = join(tmpdir(), `phi-rpc-test-${Date.now()}`);
		client = new RpcClient({
			cliPath: join(__dirname, "..", "dist", "cli.js"),
			cwd: join(__dirname, ".."),
			env: { PHI_CODING_AGENT_DIR: sessionDir },
			provider: "anthropic",
