...
import type { AgentMessage, Entry, LaneSnapshot } from "@ao-barbosa/phi-agent-core";
import type { AssistantMessage, AuthEvent } from "@ao-barbosa/phi-ai";
import {
	type Component,
	Container,
	type Focusable,
...
function runLogin(
	...
	if (!account.interactive) {
		dialog.showInfo(`${account.methodName ?? "Authentication"} is configured outside phi.`, [], true);
