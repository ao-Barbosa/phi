...
export const ENV_SERVER_DIR = "PHI_SERVER_DIR";
export const ENV_SERVER_ID = "PHI_SERVER_ID";

export function resolveServerDirectory(directory?: string): string {
	return resolvePath(directory ?? process.env[ENV_SERVER_DIR] ?? join(homedir(), ".phi", "server"));
}
...
export interface StartServerOptions {
	/** Server profile and socket directory. Defaults to PHI_SERVER_DIR or ~/.pi/server. */
	readonly directory?: string;
	/** Logical service ID. Defaults to PHI_SERVER_ID or the directory's default-server-id. */
	readonly serverId?: ServerId;
