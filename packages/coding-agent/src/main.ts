...
import { type ImageContent, modelsAreEqual } from "@ao-barbosa/phi-ai";
import { setCapabilityOverrides } from "@ao-barbosa/phi-tui";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.PHI_OFFLINE);
	if (offlineMode) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.PHI_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: PHI_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
