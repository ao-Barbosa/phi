...
import { DEFAULT_RADIUS_GATEWAY } from "@ao-barbosa/phi-ai/providers/radius-config";
import { type Container, type EditorComponent, hyperlink, type TUI } from "@ao-barbosa/phi-tui";
...
export async function shareSession(context: SessionShareContext): Promise<void> {
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "phi-share-"));
...
async function tryShareViaRadius(tmpFile: string, context: SessionShareContext): Promise<boolean> {
	...
	try {
		...
		url.searchParams.set("title", "Phi session");
