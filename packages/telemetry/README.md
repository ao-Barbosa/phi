# @ao-Barbosa/phi-telemetry

Vendor-neutral telemetry contracts and typed schema utilities for phi packages.
...
Applications can use the in-memory reference or provide an adapter for OpenTelemetry, Sentry, logs, or another backend. Phi packages pass telemetry contexts explicitly and define their domain schemas separately.
...
- [Phi Package Integration](#phi-package-integration)
...
```bash
npm install @ao-Barbosa/phi-telemetry
```
...
```typescript
import { InMemoryTelemetryContext } from '@ao-Barbosa/phi-telemetry';
...
`@ao-Barbosa/phi-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@ao-Barbosa/phi-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@ao-Barbosa/phi-agent-core';
...
## Phi Package Integration

Package ownership is intentionally split:

- `@ao-Barbosa/phi-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@ao-Barbosa/phi-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@ao-Barbosa/phi-agent-core` owns and exports the phi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
...
The phi schemas use phi-owned `phi.ai.*`, `phi.harness.*`, and `phi.session.*` names. Adapters may translate them to backend conventions without changing the emitted phi vocabulary.
