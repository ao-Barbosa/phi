...
const artifactDirectory = process.env.PHI_EVAL_ARTIFACT_DIR
	? resolve(packageRoot, process.env.PHI_EVAL_ARTIFACT_DIR)
	: resolve(
			packageRoot,
			".eval",
...
if (hasCliModelSelection) {
	...
	provider = process.env.PHI_PROVIDER?.trim() || undefined;
	model = process.env.PHI_MODEL?.trim() || undefined;
	if (Boolean(provider) !== Boolean(model)) {
		console.error("Default model selection requires both PHI_PROVIDER and PHI_MODEL.");
		process.exit(1);
	}
}
...
const childEnvironment = {
	...process.env,
	PHI_EVAL_ARTIFACT_DIR: artifactDirectory,
};
if (provider && model) {
	childEnvironment.PHI_PROVIDER = provider;
	childEnvironment.PHI_MODEL = model;
} else {
	delete childEnvironment.PHI_PROVIDER;
	delete childEnvironment.PHI_MODEL;
}
