...
export function resolveModelSelection(
	...
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both PHI_PROVIDER and PHI_MODEL as defaults.");
	}
	return { provider, id };
}
	...
	if (sessionManager) {
		try {
			...
			if (sessionPath && existsSync(sessionPath)) {
				setArtifact(PHI_SESSION_SNAPSHOT_ARTIFACT, await readFile(sessionPath, "utf8"));
			}
		} catch (error) {
