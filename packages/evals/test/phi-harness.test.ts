...
describe("resolveModelSelection", () => {
	it("prefers an explicit harness model over environment defaults", () => {
		expect(
			resolveModelSelection(
				...
				{ PHI_PROVIDER: "openai-codex", PHI_MODEL: "gpt-5.6-sol" },
			),
		).toEqual({ provider: "anthropic", id: "claude-opus-4-6" });
	});

	it("uses trimmed environment defaults when the harness has no explicit model", () => {
		expect(resolveModelSelection(undefined, { PHI_PROVIDER: " openai-codex ", PHI_MODEL: " gpt-5.6-sol " })).toEqual({
	...
	it.each([
		[undefined, {}],
		[undefined, { PHI_PROVIDER: "openai-codex" }],
		[undefined, { PHI_MODEL: "gpt-5.6-sol" }],
		[
		...
		expect(() => resolveModelSelection(explicitModel, environment)).toThrow(
			"Select a harness model explicitly or set both PHI_PROVIDER and PHI_MODEL as defaults.",
		);
	});
});
