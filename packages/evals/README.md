...
Phi evals are behavioral, model-backed checks for Phi workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native Phi session artifacts.
...
```bash
PHI_PROVIDER=openai PHI_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from Phi's normal `ModelRuntime`, including Phi subscription credentials and provider API-key
...
native Phi session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. Phi-specific evals use `createPiCodingAgentHarness(...)` from `src/phi-harness.ts`, with one harness bound
...
describeEval("Phi smoke", { harness }, (it) => {
...
- `noTools`: Phi's tool-disable configuration.
...
prompt creates or changes Phi resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create a Phi extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic Phi adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other Phi configuration:
...
The Phi harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
