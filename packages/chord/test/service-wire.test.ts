...
describe("service wire protocol", () => {
	test("encodes control calls and validates service values", () => {
		...
		expect(
			parseServiceCatalogue([
				{ serviceId: "phi.models", mode: "singleton" },
				{ serviceId: "phi.dialogs", mode: "keyed" },
			]),
		).toEqual([
			{ serviceId: "phi.models", mode: "singleton" },
			{ serviceId: "phi.dialogs", mode: "keyed" },
		]);
		const subscribe = createServiceSubscribeCall("subscription-1", "phi.models", "singleton");
		expect(decodeServiceControlCall(subscribe)).toEqual({
			type: "subscribe",
			subscriptionId: "subscription-1",
			serviceId: "phi.models",
			mode: "singleton",
		});
	...
	test("rejects malformed service values", () => {
		expect(() => parseServiceCall({ serviceId: "phi.models", member: "list", args: [], extra: true })).toThrow(
			"Invalid service call",
		);
		expect(() => parseServiceCatalogue([{ serviceId: "phi.models", mode: "unknown" }])).toThrow(
