...
describe("detectCapabilities", () => {
	...
	it("applies environment overrides", () => {
		assert.deepStrictEqual(
			withEnv({ PHI_HYPERLINKS: "1", PHI_IMAGE_PROTOCOL: "kitty", PHI_TRUE_COLOR: "1" }, () => detectCapabilities()),
			{ images: "kitty", trueColor: true, hyperlinks: true },
		);
		assert.deepStrictEqual(
			withEnv(
				{ TERM_PROGRAM: "iterm.app", PHI_HYPERLINKS: "0", PHI_IMAGE_PROTOCOL: "none", PHI_TRUE_COLOR: "0" },
				() => detectCapabilities(),
			),
	...
	it("applies and clears programmatic overrides", () => {
		withEnv({ PHI_HYPERLINKS: "1", PHI_IMAGE_PROTOCOL: "kitty", PHI_TRUE_COLOR: "1" }, () => {
	...
	it("bypasses the tmux probe when hyperlinks are overridden", () => {
		let probed = false;
		const caps = withEnv(
			{ TMUX: "/tmp/tmux-1000/default,1234,0", PHI_HYPERLINKS: "1", PHI_IMAGE_PROTOCOL: "kitty" },
			() =>
				detectCapabilities(() => {
					probed = true;
					return false;
