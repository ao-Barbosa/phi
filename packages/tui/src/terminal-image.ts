...
export function detectCapabilities(tmuxForwardsHyperlink: () => boolean = probeTmuxHyperlinks): TerminalCapabilities {
	const hyperlinks = parseBooleanCapabilityOverride(process.env.PHI_HYPERLINKS);
	const detected = detectCapabilitiesFromEnvironment(
		hyperlinks === undefined ? tmuxForwardsHyperlink : () => hyperlinks,
	);
	const imageProtocol = process.env.PHI_IMAGE_PROTOCOL?.toLowerCase();
	const images =
		imageProtocol === "kitty" || imageProtocol === "iterm2"
			...
			: imageProtocol === "none" || imageProtocol === "0"
				? null
				: undefined;
	const trueColor = parseBooleanCapabilityOverride(process.env.PHI_TRUE_COLOR);
	return {
		...detected,
...
function shortenImagePath(filename: string): string {
	const home = homedir();
	// homedir() can contain forward slashes (e.g. from $HOME) while filename uses native separators.
	const nativeHome = home.replaceAll("/", sep);
	if (
		nativeHome &&
		(filename === nativeHome || filename.startsWith(`${nativeHome}/`) || filename.startsWith(`${nativeHome}\\`))
	) {
		return `~${filename.slice(nativeHome.length)}`;
	}
	return filename;
}
